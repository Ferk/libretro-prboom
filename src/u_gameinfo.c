...
// -----------------------------------------------
static int ParseGameProperty(u_scanner_t* s, gameinfo_t *ginfo)
{
  char *pname;
  U_GetNextToken(s, TRUE);
  if (s->token != TK_Identifier) {
    U_Error(s, " Expected game property, but found '%s'", s->string);
    return 0;
  }
  ...
  if (!strcasecmp(pname, "iwad"))
  {
    ...
    U_ReplaceString(&(ginfo->iwadFile), s->string);
  }
  ...
  {
    U_MustGetToken(s, TK_StringConst);
    U_ReplaceString(&(ginfo->banner), s->string);
  }
  else
  {
    // Unknown property, skip until a new identifier is found
    while (!U_CheckToken(s, TK_Identifier) && U_HasTokensLeft(s))
      U_GetNextToken(s, TRUE);
  }
  return 1;
}
...
void U_ParseGameInfo(const byte *buffer, size_t length)
{
  u_scanner_t scanner = U_ScanOpen(buffer, length, "GAMEINFO");
...
void U_ApplyIWADInfo(const gameinfo_t *iwadinfo)
{
  if (!gameinfo.banner)
...
void U_GameInfoInit(char* wadfilename)
{
  int i;
  ...
  lprintf(LO_INFO, "U_GameInfoInit: Analizing game definitions for %s\n", wadfilename);
  ...
  if (wadlumps != NULL) {
    ...
    for (i=0; (int)i<wadheader.numlumps ; i++,lump++)
    {
      ...
      if (!strncmp(lump->name, "GAMEINFO",8)) {
       ...
       lprintf(LO_INFO, "U_GameInfoInit: parsing GAMEINFO lump\n");
       U_ParseGameInfo(data, lump->size);
      }
    }
    if(gameinfo.firstMap)
      lprintf(LO_INFO,"U_GameInfoInit: WAD's first map: %s\n", gameinfo.firstMap);
    free(wadlumps);
  }
  ...
  if (iwadinfo)
  {
    lprintf(LO_INFO, "U_GameInfoInit: using standard IWAD: '%s'\n", iwadinfo->iwadFile);
    U_ApplyIWADInfo(iwadinfo);
  }
  else if (gameinfo.iwadFile)
    lprintf(LO_INFO, "U_GameInfoInit: using non-standard IWAD: '%s'\n", gameinfo.iwadFile);
  else
    I_Error("U_GameInfoInit: Couldn't find any suitable IWAD");
}
